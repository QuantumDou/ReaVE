...
def run_sft(
    ...
    #************* 新添加的分类损失 ********************
    abnormal_ids = tokenizer.encode(" [Status: Abnormal]", add_special_tokens=False)
    normal_ids = tokenizer.encode(" [Status: Normal]", add_special_tokens=False)
    
    print("Abnormal IDs:", abnormal_ids)
    print("Normal IDs:", normal_ids)
    #*************************************************

    # MINT-CoT: interleave token initialization
    do_interleave = "interleave" in data_args.dataset[0]
    
    if not hasattr(model, 'interleave_token_id') or model.interleave_token_id is None:
        model.interleave_token_id = 151650  # <|quad_start|>
    else:
        # 确保它是整数类型，而不是张量
        if hasattr(model.interleave_token_id, 'item'):
            model.interleave_token_id = model.interleave_token_id.item()
        elif not isinstance(model.interleave_token_id, int):
            model.interleave_token_id = int(model.interleave_token_id)
    ...
    if do_interleave:
        ...
        if model.interleave_token.dim() >= 2:
            nn.init.xavier_uniform_(model.interleave_token)
        else:
            # 如果只有 1 维，改用正态分布初始化
            nn.init.normal_(model.interleave_token, std=0.02)
        nn.init.constant_(model.logit_scale, model.logit_scale_init_value/model.grad_scaler)
        for n, p in model.named_parameters():
            if 'interleave_token' in n or 's_projection' in n or 'vis_projection' in n or 'logit_scale' in n or 'decoder_status_head' in n:
    ...
    if model_args.use_kt:
        ...
        # if medical_eval_callback is not None:
        #     trainer.add_callback(medical_eval_callback)
