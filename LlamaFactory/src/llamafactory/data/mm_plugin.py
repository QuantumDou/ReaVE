...
from ..extras.constants import AUDIO_PLACEHOLDER, IGNORE_INDEX, IMAGE_PLACEHOLDER, INTERLEAVE_PLACEHOLDER, VIDEO_PLACEHOLDER
...
class Qwen2VLPlugin(BasePlugin):
    ...
    def process_messages(
        ...
        for message in messages:
            ...
            # MINT-CoT: Interleave placeholder processing
            if INTERLEAVE_PLACEHOLDER in content:
                interleave_token = "<|quad_start|>"
                selected_token = "<|quad_end|>"
                ...
                # Split content by interleave placeholders
                content_parts = content.split(INTERLEAVE_PLACEHOLDER)
                content_list = content_parts[::2]  # Text parts
                interleave_list = content_parts[1::2]  # Interleave parts (patch numbers)

                # Parse select_numbers from interleave_list
                select_numbers = []
                ...
                # NOTE:
                # 当前数据集构建阶段已经使用了与图像处理器完全一致的
                # merge_size / patch_size / 图像缩放方式，
                # 因此此处不再重复 smart_resize + patch 映射，
                # 直接将解析出的 patch 下标作为 selected_numbers 使用。
                #
                # 形状保持与原 MINT-CoT 一致: [ [step1_indices, step2_indices, ...] ]
                new_selected_numbers = [select_numbers]  # Wrap for batch processing

                # Reconstruct content with interleave and selected tokens
                interleaved_content = []
                for i, text in enumerate(content_list):
                    interleaved_content.append(text)
                    if i < len(interleave_list):
                        interleaved_content.append(interleave_token * interleave_token_num)
                        interleaved_content.append(
                            f"{self.vision_bos_token}{selected_token * len(new_selected_numbers[0][i])}{self.vision_eos_token}"
                        )

                content = "".join(interleaved_content)
                message["selected_numbers"] = new_selected_numbers
                # print("===new_selected_numbers:", new_selected_numbers)
            message["content"] = content
    ...
    def get_selected_mask(self, mm_inputs, selected_numbers, processor):
        r"""Convert selected_numbers to mask tensor for MINT-CoT training.
        
        Args:
            mm_inputs: Multi-modal inputs containing pixel_values and image_grid_thw
            selected_numbers: List of selected patch indices for each batch item
            processor: Image processor to get merge_size
            
        Returns:
            selected_mask: Tensor of shape (num_steps, num_patches) indicating selected patches
        """
    ...
    def get_mm_inputs(
        ...
        audlens: list[int],
        batch_ids: list[list[int]],
        batch_selected_numbers: list[list[list[int]]],  # MINT-CoT: 必需参数，与原项目一致
        processor: Optional["ProcessorMixin"],
    ) -> dict[str, Union[list[int], "torch.Tensor"]]:
        self._validate_input(processor, images, videos, audios)
        mm_inputs = self._get_mm_inputs(images, videos, audios, processor)
