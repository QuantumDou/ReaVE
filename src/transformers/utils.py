...
class GenerationMixin(ContinuousMixin):
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        # print("===self.config.is_encoder_decoder:",self.config.is_encoder_decoder) #false
    ...
    def _prepare_generation_config(
        ...
        **kwargs: Any,
        ...
        # NOTE (custom for Qwen2.5-VL + MINT-CoT):
        # 1）先把自定义的 interleave 相关参数从 kwargs 拷贝出来；
        # 2）先调用 GenerationConfig.update(**kwargs) 得到 model_kwargs；
        # 3）再把这些自定义参数强制写回 model_kwargs。
        custom_model_kwargs = {}
        custom_keys = [
            "interleave_inf",
            "predict_threshold",
        ...
        for _key in custom_keys:
            if _key in kwargs:
                custom_model_kwargs[_key] = kwargs[_key]

        model_kwargs = generation_config.update(**kwargs)

        # debug：看看这些 key 最终有没有进到 model_kwargs 里
        for _key in custom_keys:
            if _key in custom_model_kwargs:
                # 直接覆盖 / 写入，确保存在
                model_kwargs[_key] = custom_model_kwargs[_key]
    ...
    def generate(
        ...
        **kwargs,
        ...
        if isinstance(custom_generate, Callable):
            ...
            #GENERATION_MODES_MAPPING:{"_sample": "_sample", "_beam_search": "_beam_search", "_assisted_decoding"}
        ...
        result = decoding_method(   #这里decoding_method = _sample
            self,
            input_ids,
    ...
    def _sample(
        ...
        **model_kwargs,
        ...
        # print("===is_prefill =",is_prefill)  #第一次才输入为true
        ...
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
            ...
            next_token_scores = logits_processor(input_ids, next_token_logits) #对模型输出的原始预测分数（Logits）进行后处理和修正，以满足生成的约束条件或偏好设置。
            ...
            if model_kwargs.get("num_added_tokens") is not None: #如果有num_added_tokens的话当前的input_ids不包含手动插入的，所以要从cache中取插入之后的版本然后添加上最后一个token
                ...
                #如果已经有 predicted_labels，说明这一步是在“用选择结果重新构造序列”，直接 input_ids = new_input_ids，因为 new_input_ids 本身就已经是“重排好 + 插入好”的新完整序列。
